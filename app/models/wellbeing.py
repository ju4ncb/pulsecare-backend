...
class WellbeingEntry(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True) # Relación con usuario, clave foránea
    ...
    risk_label = relationship("RiskLabel", back_populates="entry", uselist=False, cascade="all, delete-orphan", passive_deletes=True) # Relación uno a uno con RiskLabel, permite acceder a la etiqueta de riesgo desde la entrada de bienestar
...
class ModelInputSnapshot(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    entry_id: Mapped[int] = mapped_column(ForeignKey("wellbeing_entries.id", ondelete="CASCADE"), unique=True, nullable=False, index=True) # Relación uno a uno con WellbeingEntry, clave foránea única
    ...
    entry = relationship("WellbeingEntry", back_populates="model_input", passive_deletes=True) # Relación 1:1 con WellbeingEntry, permite acceder a los datos de la entrada de bienestar desde el snapshot de entrada del modelo
...
class RiskLabel(Base):
    ...
    entry = relationship("WellbeingEntry", back_populates="risk_label", passive_deletes=True)